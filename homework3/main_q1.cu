...
// initialize an array of size arr_size in input_array with random floats between -1 and 1
void initialize_array(vec &input_array, size_t arr_size) {
    ...
    static std::uniform_real_distribution<elem_type> distribution(-1.0f, 1.0f);
...
void host_recurrence(vec &input_array, vec &output_array, size_t num_iter, size_t array_size) {
    std::transform(input_array.begin(), input_array.begin() + array_size, output_array.begin(),
        [&num_iter](elem_type &constant) {
            elem_type z = 0;
...
void check_initialization(vec &input_array, size_t arr_size) {
    ...
    for (size_t i = 0; i < arr_size; i++) {
        ...
        if (entry < -1.0 || entry > 1.0) {
            cerr << "Initialization Error: Entry " << i << " isn't between -2 and 2." << endl;
            count++;
        }
        if (count > 10) {
...
void checkResults(vec &array_host, elem_type *device_output_array, size_t num_entries) {
    // allocate space on host for gpu results
    vec array_from_gpu(num_entries);

    // download and inspect the result on the host:
    cudaMemcpy(&array_from_gpu[0], device_output_array, num_entries * sizeof(elem_type), cudaMemcpyDeviceToHost);
    ...
    for (size_t i = 0; i < num_entries; i++) {
        ...
        if (fabs(array_host[i]) > 2 && fabs((array_host[i] - array_from_gpu[i]) / array_host[i]) < 1e-4) {
            continue;
        }
...
double recurAndCheck(
    const elem_type *device_input_array,
    elem_type *device_output_array, size_t num_iter,
    size_t array_size, size_t cuda_block_size,
    size_t cuda_grid_size, vec &arr_host
) {
    // generate GPU output
    double elapsed_time = doGPURecurrence(
        device_input_array, device_output_array, num_iter, array_size, cuda_block_size, cuda_grid_size
    );
...
int main(int argc, char **argv) {
    ...
    cudaFree(nullptr); // initialize cuda context to avoid including cost in timings later

    // Warm-up each of the kernels to avoid including overhead in timing.
    // If the kernels are written correctly, then they should
    // never make bad memory access, even though we are passing in NULL
    ...
    // copy input to GPU
    cudaMemcpy(device_input_array, &init_arr[0], num_bytes, cudaMemcpyHostToDevice);
    ...
    */
    ...
    recurAndCheck(
        device_input_array, device_output_array, num_iter, array_size, cuda_block_size, cuda_grid_size, arr_host
    );
    ...
    // You can make the graph more easily by saving this array as a csv (or something else)
    ...
    */
    ...
    host_recurrence(init_arr, arr_host, num_iter, array_size);
    for (size_t cuda_block_size = 32; cuda_block_size <= 1024; cuda_block_size += 32) {
        double elapsed_time = recurAndCheck(
            device_input_array, device_output_array, num_iter, array_size, cuda_block_size, cuda_grid_size, arr_host
        );
    ...
    */
    ...
    host_recurrence(init_arr, arr_host, num_iter, array_size);
    for (size_t cuda_grid_size = 36; cuda_grid_size <= 1152; cuda_grid_size += 36) {
        double elapsed_time = recurAndCheck(
            device_input_array, device_output_array, num_iter, array_size, cuda_block_size, cuda_grid_size, arr_host
        );
        double performance = flops / (elapsed_time / 1000.) / 1E12;
