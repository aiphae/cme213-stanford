...
__global__
void offsetCopy(int n, float* odata, float* idata, int offset) {
...
__global__
void stridedCopy(int n, float* odata, float* idata, int stride) {
...
__global__
void randomCopy(int n, float* odata, float* idata, int* addr) {
    int xid = blockIdx.x * blockDim.x + threadIdx.x;
    if (xid < n && addr[xid] < n)    
